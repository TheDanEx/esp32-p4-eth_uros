...
std_msgs__msg__Int64 esp_time_msg;
...
void ping_timer_callback(rcl_timer_t * timer, int64_t last_call_time)
{
	...
	if (timer != NULL) {
		...
		// Publish ESP System Timer (Microseconds)
		esp_time_msg.data = (int64_t)esp_timer_get_time();
		rcl_publish(&time_publisher, (const void*)&esp_time_msg, NULL);

		printf("Ping send seq %s | SysTime: %lld\n", outcoming_ping.frame_id.data, esp_time_msg.data);
	}
}
...
void micro_ros_task(void * arg)
{
	...
	// Create a reliable time publisher (system telemetry)
	RCCHECK(rclc_publisher_init_default(&time_publisher, &node,
		ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int64), "/microROS/esp_time"));
